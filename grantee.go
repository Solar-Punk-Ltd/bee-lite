...
func (bl *Beelite) AddRevokeGrantees(ctx context.Context, batchHex string, granteesAddress swarm.Address, historyAddress swarm.Address, addlist, revokelist []string) (swarm.Address, swarm.Address, error) {
	...
	publisher := bl.publicKey
	ls := loadsave.New(bl.storer.Download(true), bl.storer.Cache(), requestPipelineFactory(ctx, putter, false, redundancy.DefaultUploadLevel), redundancy.DefaultUploadLevel)
	gls := loadsave.New(bl.storer.Download(true), bl.storer.Cache(), requestPipelineFactory(ctx, putter, true, redundancy.DefaultUploadLevel), redundancy.DefaultUploadLevel)
